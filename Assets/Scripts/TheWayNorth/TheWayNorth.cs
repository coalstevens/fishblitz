...
[CreateAssetMenu(fileName = "TheWayNorth", menuName = "Core/The Way North")]
public class TheWayNorth : Store
{
    [System.Serializable]
    ...
    {
        if (scene.name == StartScene.ToString() && !_isCanyonTransition)
        {
            StartRun();
            LogNetworkState();
        }
    ...
    private void StartRun()
    {
        ...
        _logger.Info($"Canyon run started at {StartScene}");
    }
    ...
    private SceneNames PickNextScene(string biomeName)
    {
        ...
        if (config == null)
        {
            Debug.LogError($"PickNextScene: No BiomeConfig named '{biomeName}'. " +
                           $"Check TheWayNorthExit._targetBiome on the Exit GO in the current scene.");
            return StartScene;
        }
    ...
    private static void ResolvePlayerSpawn(Scene scene, LoadSceneMode mode)
    {
        ...
        TheWayNorth mgr = Get<TheWayNorth>();
